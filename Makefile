...
CORE_OBJS = ds4.o ds4_image.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_ple_stream.o ds4_metal.o ds4_layer_pack.o
...
# the vendored kernels branch on __CUDA_ARCH__, so the JIT keeps their Turing
# fallbacks.  cuda/mmq/mma.cuh then emits four mma.m8n8k16.s8 in place of one
# mma.m16n8k32.s8 (measured 1.74 s vs 1.12 s of mul_mat_q per 2048-token
# qwen4exp prefill chunk on GB10), and in ds4_cuda.cu the m16n8k32 and
# ldmatrix helpers compile to stubs, which the host side detects by
# ptxVersion/binaryVersion and answers by switching the Q8_0 and Q4_K
# tensor-core tiers off.  So also build a cubin for the build host's GPU,
# keeping the compute_75 cubin *and* PTX as the portable fallback: no other
# GPU's behaviour changes, and the host-side arch queries still find 750 in
# __CUDA_ARCH_LIST__.  Blackwell needs the family-specific target because
# BLACKWELL_MMA_AVAILABLE emits block-scaled FP4 MMA, which plain sm_121
...
.PHONY: all help clean test test-server test-rocm test-glm53-kda-rocm test-metal-session-batch test-mxfp4-cuda test-mxfp4-rocm test-cuda-session-batch test-cuda-mixed-batch dspark-acceptance dspark-verify-depth mtp-verify-depth cpu cuda cuda-spark cuda-generic cuda-regression strix-halo rocm
...
strix-halo:
	$(MAKE) -B ds4 ds4-server ds4-bench ds4-eval ds4-agent \
		CORE_OBJS="ds4.o ds4_image.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_ple_stream.o ds4_rocm.o ds4_rocm_compat.o ds4_rocm_unavailable.o ds4_layer_pack.o $(ROCM_MMQ_OBJS)" \
...
test-rocm:
	$(MAKE) -B ds4_test ds4_agent_test ds4-eval q4k-dot-test tests/test_gguf_split mxfp4-dot-test \
...
# The server unit tests on their own: prompt rendering, tool-call parsing and
# repair, cache policy.  ds4_server.c's own main() under DS4_SERVER_TEST, so no
# engine, no model, and no GPU work at runtime -- `make test-server` is safe to
# run while the GPU is busy serving, unlike the full ds4_test suite.
ds4_server_test.o: ds4_server.c ds4.h ds4_ssd.h ds4_distributed.h ds4_help.h ds4_kvstore.h rax.h
	$(CC) $(CFLAGS) -DDS4_SERVER_TEST -Wno-unused-function -c -o $@ ds4_server.c

ds4_server_test: ds4_server_test.o ds4_help.o ds4_kvstore.o rax.o $(CORE_OBJS)
ifeq ($(UNAME_S),Darwin)
	$(CC) $(CFLAGS) -o $@ ds4_server_test.o ds4_help.o ds4_kvstore.o rax.o $(CORE_OBJS) $(METAL_LDLIBS)
else
	$(DS4_LINK) -o $@ ds4_server_test.o ds4_help.o ds4_kvstore.o rax.o $(CORE_OBJS) $(DS4_LINK_LIBS)
endif
...
ds4_cpu_test_hooks.o: ds4.c ds4.h ds4_image.h ds4_gpu.h ds4_gpu_mgpu.h ds4_layer_pack.h ds4_q4e_page.h
...
tests/test_engine_mgpu_placement: tests/test_engine_mgpu_placement.o ds4_cpu_test_hooks.o ds4_image.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_ple_stream.o ds4_layer_pack.o
	$(CC) $(CFLAGS) -o $@ $^ $(LDLIBS)

tests/test_qwen4exp_graph.o: tests/test_qwen4exp_graph.c ds4.h
	$(CC) $(CFLAGS) -I. -c -o $@ $<
...
tests/test_qwen4exp_tokenizer: tests/test_qwen4exp_tokenizer.o $(CORE_OBJS)
	$(DS4_LINK) -o $@ $^ $(DS4_LINK_LIBS)

tests/test_qwen4exp_ple.o: tests/test_qwen4exp_ple.c ds4.h ds4_ple_stream.h
...
tests/test_gguf_split: tests/test_gguf_split.o ds4_cpu_test_hooks.o ds4_image.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_ple_stream.o ds4_layer_pack.o
	$(CC) $(CFLAGS) -o $@ $^ $(LDLIBS)

tests/test_sampling.o: tests/test_sampling.c ds4.h
	$(CC) $(CFLAGS) -fno-finite-math-only -DDS4_TEST_HOOKS -I. -c -o $@ $<

tests/test_sampling: tests/test_sampling.o ds4_cpu_test_hooks.o ds4_image.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_ple_stream.o ds4_layer_pack.o
	$(CC) $(CFLAGS) -o $@ $^ $(LDLIBS)
...
# qwen4exp correctness against the llama.cpp oracle.  Both need the model, so
